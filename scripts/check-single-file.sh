...
<file>              path relative to the repo root (e.g. src/interval.rs)
[function_pattern]  optionally restrict verification to matching functions

examples:
  ./scripts/check-single-file.sh src/interval.rs
  ./scripts/check-single-file.sh src/runtime_interval.rs
  ./scripts/check-single-file.sh src/interval.rs lemma_add_valid
EOF
  exit 0
fi
...
#   src/interval.rs          ->  interval
#   src/runtime_interval.rs  ->  runtime_interval
...
run_cargo_verus() {
  if command -v rustup >/dev/null 2>&1; then
    ...
    cd "$ROOT_DIR"
    cargo verus verify --manifest-path Cargo.toml -p verus-interval-arithmetic \
