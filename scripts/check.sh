...
run_cargo_verus_verify() {
  ...
  if command -v rustup >/dev/null 2>&1; then
    ...
    if [[ -n "$feature_flags" ]]; then
      ...
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p verus-interval-arithmetic $feature_flags -- --triggers-mode silent
    else
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p verus-interval-arithmetic -- --triggers-mode silent
    fi
    ...
    if nix-shell -p rustup --run "rustup --version >/dev/null 2>&1" >/dev/null 2>&1; then
      ...
      nix-shell -p rustup --run "
        ...
        export VERUS_Z3_PATH='$VERUS_SOURCE/z3'
        cd '$ROOT_DIR'
        cargo $OFFLINE_CARGO_FLAG verus verify --manifest-path Cargo.toml -p verus-interval-arithmetic $feature_flags -- --triggers-mode silent
      "
    else
      skip_or_fail_verus_unavailable \
